...
names_clean <- c("Var", "ARIMA", "EWMA", "GARCH")
...
jarque.test(as.vector(ARIMA_model_m$residuals))
...
GARCH_max_m <- optimize(EWMA_function_m, interval = c(0, 1), maximum = TRUE, 
...
reg_mkt_m[[1]] <- lm(a_m[,names[1]] ~ b_m)
...
for (i in 1:length(names)) {
  ...
  reg_output_m["R^2", i] <- summary(reg_mkt_m[[i]])$r.squared
...
stargazer(reg_mkt_m[[1]], reg_mkt_m[[2]], reg_mkt_m[[3]], reg_mkt_m[[4]],
          ...
          dep.var.labels = names_clean, 
          covariate.labels = c("Market", "Alpha"),
          add.lines = list(c("R2", 
                             sprintf("%.2f", as.numeric(reg_output_m["R^2",]))),
                           c("RMSE", 
                             sprintf("%.2f", as.numeric(reg_output_m["RMSE",]))),
                           c("Sharpe Ratio", 
...
stargazer(stat_ret_m, type = "text",summary = FALSE, out = "table.htm", digits = 2)
...
ggplot(returns_m) +
  ...
  stat_density(aes(x=ARIMA_var_managed, color = "ARIMA"), 
               adjust = 1, size = line_size, geom = "line") +
  stat_density(aes(x=EWMA_var_managed, color = "EWMA"), 
               adjust = 1, size = line_size, geom = "line") +
...
for (i in 1:length(names)) {
  ...
  cost_m[i,6] <- uniroot(breakeven_function_m, strategy = i, lower = 0, upper = 100)$root
}
...
colnames(leverage_m) <- c("alpha", "100%", "50%", "0%")
...
stat_weight_m[5:11,] <- as.data.frame(apply(weights_m[,names], 2, quantile, 
...
reg_rec_m[[1]] <- lm(a_m[,names[1]] ~ b_m + b_rec_m + rec_m)
reg_rec_m[[2]] <- lm(a_m[,names[2]] ~ b_m + b_rec_m + rec_m)
...
stargazer(reg_rec_m[[1]], reg_rec_m[[2]], 
          ...
          dep.var.labels = names_clean,
          covariate.labels = c("Market", "Market * Recession", "Recession Constant"),
            type = "text", out = "table.htm", keep.stat = c("n", "rsq"), digits = 2)
...
stargazer(reg_strat_m[[1]], reg_strat_m[[2]], reg_strat_m[[3]],
          se = list(reg_strat_se_m[[1]], reg_strat_se_m[[2]],
                    reg_strat_se_m[[3]]),
          type = "text", dep.var.labels = names_clean[-1], 
          covariate.labels = c("Market", "Var"), out = "table.htm",
          keep.stat = c("n", "rsq"))
...
strat_cost_m <- data.frame(matrix(nrow = length(cost_vector), 
                                  ncol = length(names_clean[-1])))
colnames(strat_cost_m) <- names_clean[-1]
rownames(strat_cost_m)[1] <- paste(cost_vector[1] * 100, "bps")
rownames(strat_cost_m)[-1] <- paste(cost_vector[-1] * 100, "bps")
...
stargazer(strat_cost_m, type = "text", summary = FALSE, 
...
rmsd_names_m <- c("No. of obs.", "RMSD Mkt-Var ", "RMSD Mkt-ARIMA ",
...
bd_times <- bd_times %>% mutate(weight_Var = weights_m$var_managed,
                                ...
                                weight_EWMA = weights_m$EWMA_var_managed, 
...
for (frequ in min_frequ:max_frequ) {
  ...
  for (i in 1:n_days) {
    if(i %% frequ == 0) {
      ...
      var_c$var[j] <- var(FF_daily$`mkt-rf`[(i - frequ + 1):i])
    }
  }
...
colnames(var_d) <- c("date", "var")
var_d$var <- FF_daily$u_sq

var_d <- var_d %>% mutate(date = FF_daily$date)

# Calculate ARIMA Variance
variance_ts_d <- xts(var_d$var, order.by = var_d$date)

ARIMA_model_d <- auto.arima(variance_ts_d, stepwise = FALSE, approximation = FALSE)

var_d$ARIMA_var <- c(fitted(ARIMA_model_d)[-1], forecast(ARIMA_model_d, h = 1)$mean)

# Optimize EWMA Parameter
EWMA_function_d <- function(lambda) {
  EWMA_var <- c(1:n_days)
  EWMA_var[1] <- var_d$var[1]
  for(i in 2:n_days) {
    EWMA_var[i] <- lambda * EWMA_var[i-1] + (1 - lambda) * var_d$var[i]
  }
  EWMA_likelihood <- c(1:(n_days-1))
  for(i in 1:(n_days-1)) {
    EWMA_likelihood[i] <- -log(EWMA_var[i])-var_d$var[i+1]/EWMA_var[i]
  }
  return (sum(EWMA_likelihood))
}
...
GARCH_function_d <- function(alpha, beta) {
  omega <- max(0,mean(var_d$var)*(1-alpha-beta))
  GARCH_var <- c(1:n_days)
  GARCH_var[1] <- var_d$var[1]
  for(i in 2:n_days) {
    GARCH_var[i] <- omega + beta * GARCH_var[i-1] + alpha * var_d$var[i]
  }
  GARCH_likelihood <- c(1:(n_days - 1))
  for(i in 1:(n_days-1)) {
    GARCH_likelihood[i] <- -log(GARCH_var[i]) - var_d$var[i+1] / GARCH_var[i]
  }
  return (sum(GARCH_likelihood))
}
...
var_d$GARCH_var[1] <- var_d$var[1]
for (i in 2:n_days) {
  var_d$GARCH_var[i] <- omega_d + alpha_d * var_d$var[i] + 
    beta_d * var_d$GARCH_var[i-1]
}

# Calculate c with Midnight Formula
names_d <- c("ARIMA_var_managed", "EWMA_var_managed", "GARCH_var_managed")
c_d <- data.frame(matrix(ncol = length(names_d)))
colnames(c_d) <- names_d

a_qe_d <- c(1:length(names_d))
b_qe_d <- c(1:length(names_d))
c_qe_d <- c(1:length(names_d))

for (i in 1:length(names_d)) {
  a_qe_d[i] <- var(FF_daily$`mkt-rf`[-1] / var_d[-n_days,var_names[i+1]])
  b_qe_d[i] <- 2 * cov(FF_daily$`mkt-rf`[-1] / 
                         var_d[-n_days, var_names[i+1]], FF_daily$rf[-1])
  c_qe_d[i] <- var(FF_daily$rf[-1]) - var(FF_daily$mkt[-1])
  
  c_d[i] <- 1 / (2 * a_qe_d[i]) * (-b_qe_d[i] + sqrt((b_qe_d[i])^2 - 4 * 
                                                       a_qe_d[i] * c_qe_d[i]))
}

# Calculate Weights and Returns
w_abs_d <- data.frame(matrix(ncol = length(names_d), nrow = n_days - 1))
colnames(w_abs_d) <- names_d
w_abs_d[1,] <- 1
...
for (i in 2:n_days) {
  tot_ret_d$mkt[i] <- tot_ret_d$mkt[i-1] * (1 + (returns_d$mkt[i-1]/100))
...
b2_d <- trading_year * (FF_daily$HML[-1])

a_d <- as.data.frame(matrix(nrow = n_days - 1, ncol = length(names_d)))
colnames(a_d) <- names_d
for (i in 1:length(names_d)) {
  a_d[,names_d[i]] <- trading_year * (returns_d[, names_d[i]] - returns_d$rf)
}
...
for (i in 1:length(names_d)) {
  ...
  reg_output_d["R^2", i] <- summary(reg_mkt_d[[i]])$r.squared
  reg_output_d["RMSE", i] <- stats::sigma(reg_mkt_d[[i]])
  reg_output_d["SR", i] <- trading_year * 
    (mean(returns_d[,names_d[i]] - returns_d$rf)) / 
    (sqrt(trading_year) * sd(returns_d[,names_d[i]] - returns_d$rf))
...
# Output Results
alpha_stars_d <- vector(length = length(names))
for (i in 1:(length(names)-1)) {
  alpha_stars_d[i] <- ff3_alpha_stars(reg_FF3_d[[i]])
}

stargazer(reg_mkt_d[[1]], reg_mkt_d[[2]], reg_mkt_d[[3]],
          se = list(reg_mkt_se_m[[1]], reg_mkt_se_m[[2]], reg_mkt_se_m[[3]]),
          type = "text", omit.stat = "all", 
          dep.var.labels = names_clean[-1], 
          covariate.labels = c("Market", "Alpha"),
          add.lines = list(c("R2", 
                             sprintf("%.2f", as.numeric(reg_output_d["R^2",]))),
                           c("RMSE", 
                             sprintf("%.2f", as.numeric(reg_output_d["RMSE",]))),
                           c("Sharpe Ratio", 
                             sprintf("%.2f", as.numeric(reg_output_d["SR",]))),
                           c("Appr Ratio", 
                             sprintf("%.2f", as.numeric(reg_output_d["AR",]))),
                           c("Alpha FF3", 
                             paste(sprintf("%.2f", as.numeric(reg_output_d["alpha_FF3",])),
                                   alpha_stars_d, sep = "")),
                           c("", 
                             paste("(", 
                                   sprintf("%.2f", as.numeric(reg_output_d["alpha_FF3_se",])),
                                   ")", sep = ""))),
          digits = 2, out = "table.htm")

# Incorporate Transaction Cost
breakeven_function_d <- function(cost, strategy) {
...
colnames(cost_d) <- c("alpha", "|Delta w|", "1bps", "10bps", "14bps", "Break Even")
rownames(cost_d) <- names_d

for (i in 1:length(names_d)) {
  cost_d[i,1] <- breakeven_function_d(0, i)
  cost_d[i,2] <- mean(w_abs_d[,names_d[i]])
  cost_d[i,3] <- breakeven_function_d(0.01, i)
  cost_d[i,4] <- breakeven_function_d(0.1, i)
  cost_d[i,5] <- breakeven_function_d(0.14, i)
  cost_d[i,6] <- uniroot(breakeven_function_d, strategy = i, lower = 0, upper = 100)$root
}

stargazer(cost_d, type = "text", summary = FALSE, out = "test.htm")