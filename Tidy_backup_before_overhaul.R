...
# Mutate Data
n_days <- as.numeric(count(FF_daily))
n_months <- as.numeric(count(FF_monthly))
...
var_m <- var_m %>% select(date, var)
var_m <- var_m %>% mutate(vol = sqrt(var))
...
var_m$ARIMA_vol <- sqrt(var_m$ARIMA_var)
...
ggplot(var_m, aes(x = date)) +
  geom_line(aes(y = volatility * sqrt(trading_months), color = "Realized Variance")) +
...
names <- c("var_managed","ARIMA_var_managed", "EWMA_var_managed",
           "GARCH_var_managed")
...
for (i in 1:length(names)) {
  denom_m[,i] <- var_m[-n_months, var_names[i]]
}
...
for (i in 1:length(names)) {
  a_qe_m[i] <- var(FF_monthly$`Mkt-RF`[-1]/denom_m[,i])
  b_qe_m[i] <- 2*cov(FF_monthly$`Mkt-RF`[-1]/denom_m[,i], FF_monthly$RF[-1])
  c_qe_m[i] <- var(FF_monthly$RF[-1])-var(FF_monthly$mkt[-1])
  
  c_m[i] <- 1/(2*a_qe_m[i])*(-b_qe_m[i]+sqrt((b_qe_m[i])^2-4*a_qe_m[i]*c_qe_m[i]))
}
...
for (i in 1:nrow(tot_ret_m)) {
  if (i==1) {
    ...
    tot_ret_m$DD_ARIMA_var_managed[i] <- 0
    ...
    tot_ret_m$DD_ARIMA_var_managed[i] <- 
      min(0,(tot_ret_m$ARIMA_var_managed[i]-max(tot_ret_m$ARIMA_var_managed[1:i]))/
            max(tot_ret_m$ARIMA_var_managed[1:i]))
...
ggplot(tot_ret_m, aes(x = date)) +
  ...
  geom_line(aes(y=DD_ARIMA_var_managed, color = "ARIMA")) +
...
bd_times$weight_ARIMA <- weights_m$ARIMA_var_managed
...
analysis_matrix_rownames_cor <- c("No. of obs.", "Corr Mkt-Var ", "Corr Mkt-ARIMA ",
                                  "Corr Mkt-EWMA ", "Corr Mkt-GARCH ", "Corr Var-ARIMA ",
                                  "Corr Var-EWMA ", "Corr Var-GARCH ")
analysis_matrix_rownames_rmsd <- c("No. of obs.", "RMSD Mkt-Var ", "RMSD Mkt-ARIMA ",
...
# Analysis of Delta w
analysis_matrix_rownames_2 <- c("Var", "ARIMA", "EWMA", "GARCH")
...
analysis_matrix_w[2, 1] <- mean(filter(bd_times, weight > 1)$weight_ARIMA)
...
for (period in 1:length(extreme_periods)) {
  ...
  analysis_matrix_w[2, period + 1] <- mean(extreme_periods[[period]]$weight_ARIMA)
...
analysis_matrix_rel_w[2, 1] <- 
  mean(filter(bd_times, weight > 1)$weight_ARIMA - filter(bd_times, weight > 1)$weight) /
  mean(filter(bd_times, weight > 1)$weight)
...
for (period in 1:length(extreme_periods)) {
  ...
  analysis_matrix_rel_w[2, period + 1] <- 
    mean(extreme_periods[[period]]$weight_ARIMA - extreme_periods[[period]]$weight) /
    mean(extreme_periods[[period]]$weight)
...
for (frequ in min_frequ:max_frequ) {
  ...
  var_c <- var_c %>% mutate(vol = sqrt(variance))
  ...
  jarque.test(as.vector(ARIMA_model_c$residuals))
  print(ARIMA_model_c)
  ...
  denom_c[,2] <- var_c$ARIMA_var[-n_custom]
...
var_d <- var_d %>% mutate(vol = sqrt(variance), date = FF_daily$Date)
...
jarque.test(as.vector(ARIMA_model_d$residuals))
...
denom_d[,1] <- var_d$ARIMA_var[-n_days]
...
for (i in 1:(n_days-1)) {
  for (j in 1:length(names_d)) {
    ...
    if (i != 1) {
      w_abs_d[i,names_d[j]] <- abs(weights_d[i,names_d[j]] - weights_d[i-1,names_d[j]])
    }
...
stargazer(reg_output_d, type = "text", summary = FALSE, out = "test.htm")
...
breakeven_function_d <- function(cost, strategy) {
  ...
  for (i in 1:(n_days-1)) {
    returns_be_d[i] <- returns_d[i,names_d[strategy]] - 
      w_abs_d[i,names_d[strategy]] * cost
  }
  a <- 252 * (returns_be_d - returns_d$RF)
  return(lm(a~b)$coefficients[1])
}
